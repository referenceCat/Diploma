...
Использование лабораторного макета микроконтроллера позволяет исключить сборку схем на макетной плате, и обеспечить повторяемость лабораторных работ. Макет позволяет студентам быстрее перейти к программированию и проверке работы периферийных модулей. Учебный макет должен включать органы управления, устройства индикации, разъемы расширения, внешние интерфейсы и цепи питания. Для популярных зарубежных платформ доступно много готовых учебных наборов и дополнительных модулей. Для отечественного микроконтроллера MIK32 АМУР готовых лабораторных решений практически нет, поэтому аппаратные узлы учебного стенда приходится разрабатывать отдельно.

В данной работе рассматривается не весь лабораторный макет, а только одна его часть - плата пользовательского ввода. Она объединяет устройства, с помощью которых пользователь формирует входные сигналы: кнопки, переключатели, клавиатуру, поворотный энкодер и потенциометры. На этих элементах можно изучать настройку портов ввода-вывода, обработку дребезга контактов, опрос клавиатуры, применение сдвиговых регистров, работу таймеров, прерываний, аналого-цифрового преобразователя и цифровых интерфейсов SPI, USART и I2C.
...
На текущем этапе плата пользовательского ввода выполняется как самостоятельный модуль и подключается к отладочной плате с микроконтроллером с помощью проводов-перемычек. В дальнейшем разработанные решения будут перенесены на большую лабораторную плату, где пользовательский ввод будет объединен с устройствами вывода, индикацией, разъемами расширения и другими узлами учебного стенда.
